...
from xdalgorithm.toolbox.reinvent.scoring.score_summary import FinalSummary, ComponentSummary
...
class ScaffoldFilter(object, metaclass=abc.ABCMeta):
    ...
    def _update_memory(self, smile, scaffold, score=None):
        if scaffold in self.scaffolds:
            ...
            self.scaffolds[scaffold] = {smile: score}  # self.scaffolds:{scaffold1:{smiles1:score1,smiles2:score2}}
    ...
    def get_scaffolds_num(self):
        return len(self._scaffolds)

    def get_smiles_num(self):
        smi_num = 0
        for scaffold in self._scaffolds.keys():
            smi_num += len(self._scaffolds[scaffold])
        return smi_num
