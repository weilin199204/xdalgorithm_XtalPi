...
def read_smiles_file(file_path, ignore_invalid=True, num=-1, standardize=True, randomize=False,
                     max_heavy_atoms=70, min_heavy_atoms=2):
    """
    Reads a SMILES file.
    ...
    actions = []
    if standardize:
        actions.append(partial(standardize_smiles,max_heavy_atoms=max_heavy_atoms,min_heavy_atoms=min_heavy_atoms))
    if randomize:
        actions.append(randomize_smiles)
    # actions contains two functions:[standardize_smiles,randomize_smiles]
