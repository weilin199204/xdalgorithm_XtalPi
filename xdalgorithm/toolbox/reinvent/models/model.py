...
import xdalgorithm.toolbox.reinvent.models.vocabulary as mv
...
class Model:
    ...
    def load_from_file(cls, file_path: str, sampling_mode=False):
        ...
        # in case of missing "models" in sys.modules
        import sys
        if 'models' not in sys.modules:
            from xdalgorithm.toolbox.reinvent import models
        ...
        model = Model(
            ...
            start_smiles=save_dict.get('start_smiles', '')
        )
    ...
    def _sample(self, batch_size=128) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for t in self.start_smiles:
            seq = self.vocabulary[t] * torch.ones([batch_size, 1], dtype=torch.long)
