...
from xdalgorithm.toolbox.reinvent.running_modes.configurations import GeneralConfigurationEnvelope, ReinforcementLearningConfiguration
from xdalgorithm.toolbox.reinvent.running_modes.reinforcement_learning.inception import Inception
from xdalgorithm.toolbox.reinvent.running_modes.reinforcement_learning.logging import ReinforcementLogger
from xdalgorithm.toolbox.reinvent.scaffold.scaffold_filters import ScaffoldFilter
...
from xdalgorithm.toolbox.reinvent.utils import to_tensor, get_indices_of_unique_smiles

class ReinforcementRunner:
    def __init__(self, envelope: GeneralConfigurationEnvelope,
                 config: ReinforcementLearningConfiguration,
        ...
        self._agent.start_smiles = self._prior.start_smiles = start_smiles
        ...
        self._final_epoch = self._config.n_steps-1
        self._epoch_smiles_dict = {0: None,
    ...
    def run(self):
        ...
        for step in range(self._config.n_steps):
            ...
            if step in self._epoch_smiles_dict.keys():
                self._epoch_smiles_dict[step] = smiles
            # switch signs
            ...
            # The scoring is zero
            zero_scores = np.sum(score_summary.total_score==0.0)
            # The scoring is zero after scaffold penalty
            invalid_num = np.sum(score == 0.0)
            # Get the smiles number
            smiles_num_in_memory = self._scaffold_filter.get_smiles_num()
            # Get the scaffold number
            scaffold_num_in_memory = self._scaffold_filter.get_scaffolds_num()
            ...
            reset_countdown = self._stats_and_chekpoint(score, start_time, step, smiles, score_summary,
                                                        ...
                                                        zero_scores, invalid_num, smiles_num_in_memory,
                                                        scaffold_num_in_memory
                                                        )
            if step == self._final_epoch:
                self._logger._visualize_umap_figure(self._epoch_smiles_dict)
