...
import xdalgorithm.toolbox.reinvent.models.dataset as md
import xdalgorithm.toolbox.reinvent.models.model as mm
...
from xdalgorithm.toolbox.reinvent.running_modes.transfer_learning.logging.transfer_learning_logger import TransferLearningLogger
from xdalgorithm.toolbox.reinvent.utils.enums.adaptive_learning_rate_enum import AdaptiveLearningRateEnum
...
class AdaptiveLearningRate:
    ...
    def set_epoch_to_collect_smiles(self, epoch_list):
        for epoch in epoch_list:
            self._epoch_smiles_dict[epoch] = None

    def set_final_epoch(self,final_epoch):
    ...
    def collect_stats(self, epoch, model_path, training_set_path, validation_set_path=None):
        ...
        if epoch in self._epoch_smiles_dict.keys():
            self._epoch_smiles_dict[epoch] = sampled_smiles
        ...
        if (self._final_epoch is not None) and (epoch == self._final_epoch):
